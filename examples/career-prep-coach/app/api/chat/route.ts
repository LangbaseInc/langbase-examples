...
async function handleCreateMemory(req: Request, userApiKey: string) {
  ...
  const response = await fetch('https://api.langbase.com/v1/memory', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
async function handleFileUpload(
  ...
  if (!file || !memoryName) {
    return new Response(
      JSON.stringify({ error: 'Invalid file or memory name' }),
      { status: 400 }
    )
  }
  ...
  if (uploadResponse.ok) {
    const pipeUpdateResponse = await updatePipe(
      ownerLogin,
      memoryName,
      userApiKey
    )
...
async function updatePipe(
  ...
  const url = `https://api.langbase.com/v1/pipes/shoes-expert`

  const pipe = {
    name: 'shoes-expert',
    description:
      'An AI-powered shoe expert that recommends Nike and Adidas footwear based on customer preferences and provides personalized shopping assistance.',
    status: 'private',
    config: {
